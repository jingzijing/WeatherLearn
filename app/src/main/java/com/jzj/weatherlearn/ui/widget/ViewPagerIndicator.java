...
public class ViewPagerIndicator extends View
{
	...
	public ViewPagerIndicator(Context context, @Nullable AttributeSet attrs, int defStyleAttr)
	{
		...
		TypedArray typedArray = context.getTheme()
				.obtainStyledAttributes(attrs, R.styleable.Indicator, defStyleAttr, 0);
	...
	protected void onDraw(Canvas canvas)
	{
		...
		for (int i = 0; i < totalIndex; i++) {
			RectF rectF = new RectF(tempX - radius, startY, tempX + radius, startY + 2 * radius);
			//选中圆点
			if (i == currentIndex) {
				paint.setColor(selectedColor);
				rectF = new RectF(tempX - selectRadius, startSelectedY, tempX + selectRadius, startSelectedY + 2 * selectRadius);
			}
